...
import { determineArchetype } from '@/lib/data/archetypes';
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-8">
      <div className="text-center space-y-4">
        <span className="text-6xl block mb-4">{archetype.icon}</span>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {(Object.entries(stats) as [StatType, number][]).map(([stat, value]) => (
          <div
            key={stat}
            className={\`p-4 rounded-xl border-2 \${
              archetype.stats[stat] > 0
...
function getStatIcon(stat: StatType): string {
